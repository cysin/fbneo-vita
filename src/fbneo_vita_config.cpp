...
FBNeoVitaConfig::FBNeoVitaConfig(Renderer *renderer, const int version) : PEMUConfig(renderer, "FBNEO_VITA", version) {
    printf("FBNeoVitaConfig(%s, v%i)\n", getPath().c_str(), version);
    ...
    if (!group) {
        printf("FBNeoVitaConfig: error, group not found (MENU_ROM_OPTIONS)\n");
        return;
    }
    group->addOption(
    ...
    // reload config now so the new rom paths are immediately visible to the file browser
    PEMUConfig::load();

    // add custom rom path
    addRomPath("ARCADE", io->getDataPath() + "arcade/", {HARDWARE_PREFIX_ARCADE, 0, "Arcade"});
#ifndef __FBNEO_VITA_LIGHT__

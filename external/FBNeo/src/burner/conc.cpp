...
#define INSIDE_NOTHING (0xFFFF & (1 << ((sizeof(TCHAR) * 8) - 1)))
	...
	printf("ConfigParseFile: fopen('%s') = %p\n", pszFilename, (void*)h);
	if (h == NULL) {
		return 1;
	}
...
INT32 ConfigCheatLoad()
{
	...
	printf("ConfigCheatLoad: szAppCheatsPath = '%s'\n", szAppCheatsPath);
	printf("ConfigCheatLoad: DRV_NAME = '%s'\n", BurnDrvGetText(DRV_NAME));

	if (ConfigParseMAMEFile()) {
		_stprintf(szFilename, _T("%s%s.ini"), szAppCheatsPath, BurnDrvGetText(DRV_NAME));
		printf("ConfigCheatLoad: trying ini '%s'\n", szFilename);
		if (ConfigParseFile(szFilename)) {
			printf("ConfigCheatLoad: ini failed, trying dat\n");
			_stprintf(szFilename, _T("%s%s.dat"), szAppCheatsPath, BurnDrvGetText(DRV_NAME));
			if (ConfigParseNebulaFile(szFilename)) {
				printf("ConfigCheatLoad: dat also failed\n");
				return 1;
			}
		} else {
			printf("ConfigCheatLoad: ini parsed OK\n");
		}
	} else {
		printf("ConfigCheatLoad: MAME cheat.xml parsed OK\n");
	}

	printf("ConfigCheatLoad: pCheatInfo = %p\n", (void*)pCheatInfo);
